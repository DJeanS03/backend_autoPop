...
$app->put('/change-password-motoboy', function (Request $request, Response $response, $args) {
  ...
  if (stripos($authHeader, 'Bearer ') === 0) {
    $jwtRaw = trim(substr($authHeader, 7));
  }
  $decoded = JWT::decode($jwtRaw, new Key($this->get('secret'), $this->get('crypt-type')));
  ...
  $check = verify_and_upgrade($input['senha_atual'], $resCheckPassword->senha);
  if (!$check['ok']) {
    $response->getBody()->write(json_encode(['msg' => 'Senha atual inválida']));
    return $response->withHeader('Content-Type', 'application/json')->withStatus(403);
  }

  // rehash da senha atual, se recomendado (opcional)
  if (!empty($check['rehash'])) {
    try {
      $updHash = $this->get('db')->prepare("UPDATE cadastro_motoboy SET senha = :hash WHERE id = :id");
      $updHash->execute(['hash' => $check['rehash'], 'id' => $resCheckPassword->id]);
    } catch (PDOException $e) { /* ignora */
    }
  }

  // nova senha (bcrypt helper)
  ...
  emailSend('contato@autopop.com.br', 'contato@autopop.com.br', $resCheckPassword->email, 'Alteração de senha: Motoboy - Autopop', $emailBody);
