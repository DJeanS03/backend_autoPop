...
$app->post('/auth-client', function (Request $request, Response $response, $args){
  ...
  //Verifica se a senha bate com o digitado
...
$app->post('/create-client-account', function(Request $request, Response $response, $args) use($app){
  ...
  try{
    $verifyExistence = $this->get('db')->prepare("SELECT id FROM cadastro_cliente WHERE cpf = :cpf OR email = :email");
    $verifyExistence->bindParam('cpf', $input['cpf']);
