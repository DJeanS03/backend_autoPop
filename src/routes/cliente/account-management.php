...
require_once __DIR__."/../../templates/emailDeactivateClient.php";
require_once __DIR__."/../../templates/emailDeactivatedAccount.php";

$app->post('/deactivationrequest-client', function(Request $request, Response $response, $args){
  $decoded = JWT::decode($request->getHeaderLine('Authorization'), new Key($this->get('secret'), $this->get('crypt-type')));

  try{
    $dadosUsuario = $this->get('db')->prepare("SELECT id, email, senha FROM cadastro_cliente WHERE id = :id");
  ...
  $emailBody = emailDeactivateClient($data_email);

  emailSend('contato@autopop.com.br', 'contato@autopop.com.br', $decoded->email, 'Requisição de desativação de conta: Cliente - Autopop', $emailBody);
...
$app->post('/deactivateaccount-verifycode-client', function(Request $request, Response $response, $args){
...
$app->delete('/deactivateaccount-client/{id}', function(Request $request, Response $response, $args){
  $id = $args['id'];
  $decoded = JWT::decode($request->getHeaderLine('Authorization'), new Key($this->get('secret'), $this->get('crypt-type')));

  try{
    $deactivateAccount = $this->get('db')->prepare("UPDATE cadastro_cliente SET email_confirmado = 0 WHERE id = :id");
  ...
  emailSend('contato@autopop.com.br', 'contato@autopop.com.br', $decoded->email, "Conta desativada - Cliente: Autopop", $emailBody);

  $response->getBody()->write(json_encode(['msg' => 'Usuário desativado com sucesso. Para reativá-lo, entre em contato com o suporte']));
  return $response->withHeader('Content-Type', 'application/json')->withStatus(200);
});